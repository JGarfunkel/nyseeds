...
export class JsonFileStorage implements IStorage {
  ...
  async getStatutes(): Promise<Statute[]> {
    ...
    for (const municipality of municipalities) {
      for (const domain of domains) {
        ...
        const directoryName = this.getMunicipalityDirectoryName(municipality.id);
  ...
  async getStatuteByMunicipalityAndDomain(municipalityId: string, domainId: string, realmId?: string): Promise<Statute | undefined> {
    ...
    const directoryName = this.getMunicipalityDirectoryName(municipalityId);
    const metadataPath = path.join(this.dataDir, dataPath, domainId, directoryName, 'metadata.json');
    ...
    if (await fs.pathExists(metadataPath)) {
      try {
        ...
        // Continue with original municipalityId if metadata read fails
      }
    }
    
    // Check for statute or policy file directly in realm-specific path
    const targetDirectoryName = this.getMunicipalityDirectoryName(targetMunicipalityId);
    const statuteFile = path.join(this.dataDir, dataPath, domainId, targetDirectoryName, 'statute.txt');
    const policyFile = path.join(this.dataDir, dataPath, domainId, targetDirectoryName, 'policy.txt');
    
    
    // Try statute file first, then policy file
    let contentFile = statuteFile;
    if (!await fs.pathExists(statuteFile) && await fs.pathExists(policyFile)) {
      contentFile = policyFile;
    }
    ...
    if (await fs.pathExists(contentFile)) {
      ...
      return {
        ...
        content,
        sourceUrl: null,
        lastUpdated: (await fs.stat(contentFile)).mtime.toISOString()
      };
    }
  ...
  // Helper to map municipality ID to directory name (handles hyphen mismatches)
  getMunicipalityDirectoryName(municipalityId: string): string {
    // Special cases for municipalities with hyphen mismatches and TownVillage to Town conversion
  ...
  async getAnalysesByMunicipalityAndDomain(municipalityId: string, domainId: string): Promise<Analysis[]> {
    ...
    // Use the directory mapping to handle hyphen mismatches
    const directoryName = this.getMunicipalityDirectoryName(municipalityId);
    const analysisFile = path.join(
      this.dataDir,
      domain.id, // Use domain.id instead of domain.name
      directoryName, // Use mapped directory name
      'analysis.json'
    );
  ...
  async getMunicipalityDomains(municipalityId: string): Promise<MunicipalityDomain[]> {
    ...
    for (const domain of allDomains) {
      const directoryName = this.getMunicipalityDirectoryName(municipalityId);
      const statuteDir = path.join(this.dataDir, 'environmental-municipal', domain.id, directoryName);
  ...
  async getDomainSummary(domainId: string, realmId?: string): Promise<Array<{municipalityId: string, grade: string | null, gradeColor: string | null, available: boolean, stateCodeApplies: boolean}>> {
    ...
    for (const municipality of municipalities) {
      const directoryName = this.getMunicipalityDirectoryName(municipality.id);
      const statuteDir = path.join(this.dataDir, dataPath, domainId, directoryName);
      const statuteExists = await fs.pathExists(path.join(statuteDir, 'statute.txt'));
